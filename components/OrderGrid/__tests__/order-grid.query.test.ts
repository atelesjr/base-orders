...
describe('order-grid.query', () => {
	...
	it('falls back page to 1 when page is NaN', () => {
		expect(resolveOrderGridQuery({ page: 'abc' }).requestedPage).toBe(1);
		expect(resolveOrderGridQuery({ page: 'NaN' }).requestedPage).toBe(1);
	});

	it('returns page 1 when page param is absent', () => {
		const result = resolveOrderGridQuery({ sortBy: 'price' });

		expect(result.requestedPage).toBe(1);
	});

	it('returns all three resolved fields together', () => {
		const result = resolveOrderGridQuery({ page: '3', sortBy: 'timestamp', sortDir: 'desc' });
